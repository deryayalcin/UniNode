...
import { verifyStudentCard } from '../services/geminiService';
...
const aiResult = await verifyStudentCard(files.cardImage[0].path);
const aiVerificationStatus = aiResult.isStudentCard
  ? aiResult.confidence === 'high' ? 'ai_approved' : 'manual_review'
  : 'ai_rejected';

if (!aiResult.isStudentCard && aiResult.confidence === 'high') {
  fs.unlinkSync(files.cardImage[0].path);
  res.status(400).json({
    error: 'The uploaded image does not appear to be a student ID card. Please upload a valid student ID.',
